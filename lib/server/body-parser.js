        ...
        switch (options.type) {
            ...
            case 'form': {
                await this._receiveData(req,options).then(function (result) {
                    ...
                    rel.result = that.yuri2.yuri2Format.queryToJson(query);
                }).catch(function (err) {
                    rel.err=err;
                    rel.result={};
                });
            ...
            case 'json': {
                await this._receiveData(req,options).then(function (result) {
                    let query = result.toString(options.encoding);
                    rel.result = that.yuri2.yuri2Format.stringToJson(query);
                }).catch(function (err) {
                    rel.err=err;
                    rel.result={};
                });
