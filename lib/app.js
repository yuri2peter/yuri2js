...
class Yuri2App extends Event {
    ...
    listen() {
        ...
        const fnOnrequest=async (req, res) => {
        ...
        if(this.data.ssl){
            this.server = https.createServer(this.data.ssl,fnOnrequest).listen(that.data.port);
        }else{
            this.server = http.createServer(fnOnrequest).listen(that.data.port);
        }
        this.server.on('listening',function () {
            console.log(`App "${that.data.name}" is running on http${that.data.ssl?'s':''}://localhost:${that.data.port}`);
            that.emit('start');
        });
    ...
    stop() {
        if (this.state.runningState==='running') {
            ...
            this.emit('close');
            console.log(`The app "${this.data.name}" on ${this.serverPort} was closed.`);
        }
    }
